...
void UEnhancedOnlineSessionsSubsystem::LoginOnlineUser(UEnhancedOnlineRequest_LoginUser* Request, bool bAllowCommandLineAuth)
{
	if (Request == nullptr)
	{
	...
	LoginOnlineUserInternal(LocalPlayer, Request, bAllowCommandLineAuth);
}

void UEnhancedOnlineSessionsSubsystem::LoginOnlineUserInternal(ULocalPlayer* LocalPlayer, UEnhancedOnlineRequest_LoginUser* Request, bool bAllowCommandLineAuth)
{
	...
	FString ParsedAuthType, ParsedToken, ParsedId;
	FParse::Value(FCommandLine::Get(), TEXT("AUTH_TYPE="), ParsedAuthType);
	FParse::Value(FCommandLine::Get(), TEXT("AUTH_LOGIN="), ParsedToken);
	FParse::Value(FCommandLine::Get(), TEXT("AUTH_PASSWORD="), ParsedId);
		
	FOnlineAccountCredentials Credentials;

	if (bAllowCommandLineAuth && !ParsedAuthType.IsEmpty())
	{
		Credentials.Type = ParsedAuthType;
		Credentials.Token = ParsedToken;
		Credentials.Id = ParsedId;
	}
	else
	{
