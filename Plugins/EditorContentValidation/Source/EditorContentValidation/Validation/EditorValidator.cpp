...
void UEditorValidator::ValidateCheckedOutContent(bool bInteractive, const EDataValidationUsecase InValidationUsecase)
{
	...
	if (AssetRegistryModule.Get().IsLoadingAssets())
	{
		...
		{
			UE_LOG(LogEditorContentValidation, Display, TEXT("Could not run ValidateCheckedOutContent because asset discovery was still being done."));
		}
		return;
	}
	...
	TArray<FString> AllWarningsAndErrors;
	{
		...
		}
		{
			...
			SlowTask.MakeDialog();
			if (!ValidatePackages(ChangedPackageNames, DeletedPackageNames, VALIDATION_MAX_PACKAGES_TO_LOAD, AllWarningsAndErrors, InValidationUsecase))
			{
						...
						if (!AssetData.GetTagValueRef<bool>(FBlueprintTags::IsDataOnly) && !UEditorValidator::IsInUncookedFolder(PackageName))
						{
							...
							if (!ClassFromData.IsEmpty())
							{
								...
								if (DerivedClassNames.Contains(ClassObjectPath))
								{
									...
									{
										bTooManyFiles = true;
										UE_LOG(LogEditorContentValidation, Display, TEXT("Too many assets invalidated (Max %d) by change to, %s + %s"), GMaxAssetsChangedByAHeader, *ChangedHeaderModule, *ChangedHeaderReleativeToModule);
