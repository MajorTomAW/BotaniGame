...
UCommonActivatableWidget* UCommonUIExtensions::PushContentToLayer_ForPlayer(const ULocalPlayer* LocalPlayer, FGameplayTag LayerName, TSubclassOf<UCommonActivatableWidget> WidgetClass)
{
	...
	{
		UE_LOG(LogCommonGame, Warning, TEXT("PushContentToLayer_ForPlayer: Invalid parameters"));
		return nullptr;
	}
	...
	if (UGameUIManagerSubsystem* UIManager = LocalPlayer->GetGameInstance()->GetSubsystem<UGameUIManagerSubsystem>())
	{
		if (UGameUIPolicy* Policy = UIManager->GetCurrentUIPolicy())
		{
			...
			UE_LOG(LogCommonGame, Warning, TEXT("PushContentToLayer_ForPlayer: Failed due to: \n UIManagerSubsystem: %s \n Policy: %s \n RootLayout: %s"),
				UIManager ? *UIManager->GetName() : TEXT("nullptr"),
				UIManager ? *UIManager->GetCurrentUIPolicy()->GetName() : TEXT("nullptr"),
				UIManager ? *UIManager->GetCurrentUIPolicy()->GetRootLayout(CastChecked<UCommonLocalPlayer>(LocalPlayer))->GetName() : TEXT("nullptr"));
		}
	}
