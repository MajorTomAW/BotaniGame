...
#include "BotaniWidgetBlueprintFactory.h"
...
#define LOCTEXT_NAMESPACE "UBotaniWidgetBlueprintFactory"
...
public:
	...
	/** Disallowed children of these classes will be excluded. */
	...
	virtual bool IsClassAllowed(const FClassViewerInitializationOptions& InInitOptions, const UClass* InClass, TSharedRef< FClassViewerFilterFuncs > InFilterFuncs) override
	{
		return !InClass->HasAnyClassFlags(DisallowedClassFlags)
			...
			&& InFilterFuncs->IfInClassesSet(DisallowedChildrenOfClasses, InClass) != EFilterReturn::Passed;
	}
	...
	virtual bool IsUnloadedClassAllowed(const FClassViewerInitializationOptions& InInitOptions, const TSharedRef< const IUnloadedBlueprintData > InUnloadedClassData, TSharedRef< FClassViewerFilterFuncs > InFilterFuncs) override
	{
		return !InUnloadedClassData->HasAnyClassFlags(DisallowedClassFlags)
			...
			&& InFilterFuncs->IfInClassesSet(DisallowedChildrenOfClasses, InUnloadedClassData) != EFilterReturn::Passed;
	}

};

UBotaniWidgetBlueprintFactory::UBotaniWidgetBlueprintFactory(const FObjectInitializer& ObjectInitializer)
    : Super(ObjectInitializer)
{
	bCreateNew = true;
	bEditAfterNew = true;
	SupportedClass = UBotaniUserWidget::StaticClass();
	ParentClass = nullptr;
}

bool UBotaniWidgetBlueprintFactory::ConfigureProperties()
{
	if (GetDefault<UUMGEditorProjectSettings>()->bUseUserWidgetParentClassViewerSelector || GetDefault<UUMGEditorProjectSettings>()->bUseUserWidgetParentDefaultClassViewerSelector)
	{
		...
		Filter->DisallowedChildrenOfClasses.Add(UUserWidget::StaticClass());

		const FText TitleText = LOCTEXT("CreateWidgetBlueprint", "Pick Parent Class for New Botani Widget Blueprint");
	...
	if (GetDefault<UUMGEditorProjectSettings>()->bUseWidgetTemplateSelector)
	{
		...
		const FText TitleText = LOCTEXT("CreateRootWidgetBlueprint", "Pick Root Widget for New Botani Widget Blueprint");
...
bool UBotaniWidgetBlueprintFactory::ShouldShowInNewMenu() const
{
	return true;
}

UObject* UBotaniWidgetBlueprintFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name,
