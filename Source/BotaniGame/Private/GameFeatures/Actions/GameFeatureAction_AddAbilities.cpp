...
EDataValidationResult UGameFeatureAction_AddAbilities::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		...
		for (const FBotaniAbilityGrant& Ability : Entry.GrantedAbilities)
		{
		...
		for (const FBotaniAttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
		...
		for (const TSoftObjectPtr<const UBotaniAbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		{
...
void UGameFeatureAction_AddAbilities::AddToWorld(const FWorldContext& WorldContext, const FGameFeatureStateChangeContext& ChangeContext)
{
		...
		(World->IsGameWorld()))
	{
		...
		for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
		{
			...
			TSharedPtr<FComponentRequestHandle> ExtensionRequestHandle = ComponentManager->AddExtensionHandler(Entry.ActorClass.Get(), AddAbilitiesDelegate);
...
void UGameFeatureAction_AddAbilities::AddActorAbilities(AActor* Actor, const FGameFeatureAbilitiesEntry& AbilitiesEntry, FPerContextData& ActiveData)
{
	...
	// Only server can add abilities
	if (!Actor->HasAuthority())
	{
		return;
	}

	// Early out if we already have abilities for this actor
	...
	{
		UE_LOG(LogGameFeatures, Error, TEXT("Failed to add UAbilitySystemComponent to Actor %s. Abilities will not be granted"), *Actor->GetPathName());
		return;
	}
	...
	// Grant abilities
	for (const auto& Ability : AbilitiesEntry.GrantedAbilities)
	{
		if (Ability.AbilityType.IsNull())
		{
			continue;
		...
		FGameplayAbilitySpecHandle Handle = AbilitySystemComponent->GiveAbility(NewSpec);

		AddedExtensions.Abilities.Add(Handle);
	}

	// Grant attributes
	for (const auto& Attributes : AbilitiesEntry.GrantedAttributes)
	{
		if (Attributes.AttributeSetType.IsNull())
		{
			continue;
		...
		if (TSubclassOf<UAttributeSet> SetClass = Attributes.AttributeSetType.LoadSynchronous())
		{
	...
	UBotaniAbilitySystemComponent* BotaniASC = CastChecked<UBotaniAbilitySystemComponent>(AbilitySystemComponent);
	for (const TSoftObjectPtr<const UBotaniAbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
	{
		if (const UBotaniAbilitySet* Set = SetPtr.Get())
		{
			Set->GiveToAbilitySystem(BotaniASC, &AddedExtensions.AbilitySetHandles.AddDefaulted_GetRef());
		}
	}
...
void UGameFeatureAction_AddAbilities::RemoveActorAbilities(AActor* Actor, FPerContextData& ActiveData)
{
	...
	if (UAbilitySystemComponent* AbilitySystemComponent = Actor->FindComponentByClass<UAbilitySystemComponent>())
	{
		...
		for (const FGameplayAbilitySpecHandle& Handle : ActorExtensions->Abilities)
		{
		...
		{
			SetHandle.TakeFromAbilitySystem(BotaniASC);
		}
	}
