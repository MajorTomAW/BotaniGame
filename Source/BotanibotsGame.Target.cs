...
public class BotanibotsGameTarget : TargetRules
{
	public BotanibotsGameTarget(TargetInfo Target) : base(Target)
	{
		...
		ApplySharedBotaniTargetSettings(this);
	}

	private static bool bHasWarnedAboutShared = false;

	internal static void ApplySharedBotaniTargetSettings(TargetRules Target)
	{
		ILogger Logger = Target.Logger;
		Target.bUseLauncherChecks = true;
		...
		else
		{
			...
			else
			{
				if (!bHasWarnedAboutShared)
				{
					...
					Logger.LogWarning("Shared Botani target settings are not applied to {0} target {1}", Target.Type, Target.Name);
				}
			}
	...
	static public bool ShouldEnableAllGameFeatures(TargetRules Target)
	{
		if (Target.Type == UnrealBuildTool.TargetType.Editor)
		{
			// @see LyraStartGame
	...
	static public void ConfigureGameFeaturePlugins(TargetRules Target)
	{
		...
		bool bBuildAllGameFeaturePlugins = ShouldEnableAllGameFeatures(Target);
