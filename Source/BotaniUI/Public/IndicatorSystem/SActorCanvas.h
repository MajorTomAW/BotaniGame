...
/** An actor canvas is a panel that can be used to render widgets in world space. */
...
public:
	...
	{
	public:
		FSlot(UBotaniIndicatorDescriptor* InIndicator)
		: TSlotBase<FSlot>()
	...
	private:
		...
		UBotaniIndicatorDescriptor* Indicator;
...
private:
	void OnIndicatorAdded(UBotaniIndicatorDescriptor* Indicator);
	void OnIndicatorRemoved(UBotaniIndicatorDescriptor* Indicator);

	void AddIndicatorForEntry(UBotaniIndicatorDescriptor* Indicator);
	void RemoveIndicatorForEntry(UBotaniIndicatorDescriptor* Indicator);

	using FScopedWidgetSlotArguments = TPanelChildren<FSlot>::FScopedWidgetSlotArguments;
	FScopedWidgetSlotArguments AddActorSlot(UBotaniIndicatorDescriptor* Indicator);
	...
	void GetOffsetAndSize(const UBotaniIndicatorDescriptor* Indicator,
		FVector2D& OutSize, 
		FVector2D& OutOffset,
...
private:
	TArray<TObjectPtr<UBotaniIndicatorDescriptor>> AllIndicators;
	TArray<UBotaniIndicatorDescriptor*> InactiveIndicators;
	
	FLocalPlayerContext LocalPlayerContext;
	TWeakObjectPtr<UBotaniIndicatorManagerComponent> IndicatorComponentPtr;
