...
#include "IndicatorSystem/Interfaces/IndicatorWidgetInterface.h"
...
// Offsets for each direction of the arrow
const FVector2D ArrowOffsets[EArrowDirection::MAX] =
	{
		FVector2D(-1.0f, 0.0f), // Left
		FVector2D(0.0f, -1.0f), // Top
		FVector2D(1.0f, 0.0f), // Right
		FVector2D(0.0f, 1.0f) // Bottom
	};
...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	UBotaniIndicatorManagerComponent* IndicatorComponent = IndicatorComponentPtr.Get();
	if (IndicatorComponent == nullptr)
	{
		IndicatorComponent = UBotaniIndicatorManagerComponent::GetComponent(LocalPlayerContext.GetPlayerController());
		if (IndicatorComponent)
		{
			...
			for (UBotaniIndicatorDescriptor* Indicator : IndicatorComponent->GetIndicators())
			{
	...
	if (LocalPlayer)
	{
		...
		if (LocalPlayer->GetProjectionData(LocalPlayer->ViewportClient->Viewport, /*out*/ ProjectionData))
		{
			...
			for (int32 ChildIndex = 0; ChildIndex < CanvasChildren.Num(); ++ChildIndex)
			{
				...
				UBotaniIndicatorDescriptor* Indicator = CurChild.Indicator;
...
void SActorCanvas::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	if (bShowAnyIndicators)
	{
		...
		for (int32 ChildIndex = 0; ChildIndex < SortedSlots.Num(); ++ChildIndex)
		{
			...
			const UBotaniIndicatorDescriptor* Indicator = CurChild.Indicator;
...
void SActorCanvas::OnIndicatorAdded(UBotaniIndicatorDescriptor* Indicator)
{
...
void SActorCanvas::OnIndicatorRemoved(UBotaniIndicatorDescriptor* Indicator)
{
	RemoveIndicatorForEntry(Indicator);
...
void SActorCanvas::AddIndicatorForEntry(UBotaniIndicatorDescriptor* Indicator)
{
	...
	{
		TWeakObjectPtr<UBotaniIndicatorDescriptor> IndicatorPtr(Indicator);
		AsyncLoad(IndicatorClass, [this, IndicatorPtr, IndicatorClass]() {
			if (UBotaniIndicatorDescriptor* Indicator = IndicatorPtr.Get())
			{
...
void SActorCanvas::RemoveIndicatorForEntry(UBotaniIndicatorDescriptor* Indicator)
{
...
SActorCanvas::FScopedWidgetSlotArguments SActorCanvas::AddActorSlot(UBotaniIndicatorDescriptor* Indicator)
{
...
void SActorCanvas::GetOffsetAndSize(const UBotaniIndicatorDescriptor* Indicator,
	FVector2D& OutSize, 
	FVector2D& OutOffset,
	...
	FVector2D& OutPaddingMax) const
{
	...
	switch(Indicator->GetHorizontalAlignment())
	{
	...
	switch(Indicator->GetVerticalAlignment())
	{
		case VAlign_Top:
			OutOffset.Y = 0.0f;
